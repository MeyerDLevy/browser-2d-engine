import { MAP_SIZE, TICK_HZ, makeWorld, type World } from '../shared/world.ts'
...
onkeydown = ev => {
  if (!playing() || (ev.target as HTMLElement).tagName === 'INPUT') return
...
function connect(room: string, playerName: string) {
  lobby = slug(room)
  ...
  prev.clear()
  meId = ''
  history.replaceState(null, '', '?lobby=' + encodeURIComponent(lobby) + '&name=' + encodeURIComponent(name))
  overlay.classList.add('off')
  const proto = location.protocol === 'https:' ? 'wss' : 'ws'
  if (ws) { ws.onclose = null; ws.close() }
  const sock = new WebSocket(proto + '://' + location.host)
  ws = sock
  sock.onopen = () => sock.send(JSON.stringify({ type: 'join', lobby, name }))
  sock.onmessage = ev => {
...
async function refreshRooms() {
  ...
  for (const r of list) {
    ...
    row.innerHTML = '<span>' + r.id + '</span><span>' + r.players + '/' + r.cap + '</span>'
    if (r.players < r.cap) row.onclick = () => connect(r.id, nameEl.value)
    roomsEl.appendChild(row)
  }
}

goEl.onclick = () => connect(roomEl.value || nameEl.value || 'room', nameEl.value)
...
if (params.get('lobby')) {
  ...
  connect(params.get('lobby'), nameEl.value)
}
...
;(window as any).G = { live, cam, world, keys, get ws() { return ws } }
